...
from .transformer_eta import build_transformer_eta
from .utils import MLP, ContrastiveEmbed, sigmoid_focal_loss

from .matcher import build_matcher
from .adapter_attention import *
import ipdb

from preprocess.preprocessing_function import *
...
class GroundingDINO_Eta(nn.Module):
    ...
    def __init__(
        ...
        if self.args.llm_modelname == "InternVL2-2B":
            self.hidden_state_dim = 2048
        elif self.args.llm_modelname == "InternVL2-1B":
            self.hidden_state_dim = 896
        elif self.args.llm_modelname == "InternVL2-8B":
            self.hidden_state_dim = 4096

        if self.args.llm_modelname == "InternVL2-2B" and self.args.encoder_type in ["upsample_cat", "hidden_states"]:
            self.fc_proj_shuffle = nn.ModuleList([
                nn.Conv2d(in_channels=512, out_channels=192, kernel_size=1, stride=1),
                nn.Conv2d(in_channels=512, out_channels=384, kernel_size=3, stride=2, padding=1),
                nn.Sequential(
                    nn.Conv2d(in_channels=512, out_channels=768, kernel_size=3, stride=2, padding=1),
                    nn.Conv2d(in_channels=768, out_channels=768, kernel_size=3, stride=2, padding=1)
                )
            ])
        elif self.args.llm_modelname == "InternVL2-2B" and self.args.encoder_type in ["fusion"]:
            self.resz_internvl_hidden_states = nn.Linear(512, 256)

        if self.args.encoder_type == "fusion":
            self.alpha = nn.Parameter(torch.tensor(0.0))
            self.resz_hidden_states = nn.Linear(1024, 256)
            self.cross_attn_src_hidden = nn.MultiheadAttention(embed_dim=256, num_heads=16, batch_first=True)
        
        if self.args.encoder_type == "upsample_cat":
            self.conv_hidden = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=1, stride=1)
            self.alpha = nn.Parameter(torch.tensor(0.0))

        if self.args.encoder_type == "adapter":
            self.conv_img_embed = nn.Conv2d(in_channels=self.hidden_state_dim , out_channels=256, kernel_size=3, stride=2, padding=1)
            self.cross_attn_hidden_text = nn.MultiheadAttention(embed_dim=self.hidden_state_dim, num_heads=16, batch_first=True, dropout=0.0)
            # self.norm_eps = 1e-5
            # self.dim = self.hidden_state_dim 
            # self.ffn_norm = RMSNorm(self.dim, eps=self.norm_eps)

        if self.args.encoder_type == "img_embed_adapter":
            self.alpha = nn.Parameter(torch.tensor(0.0))
            self.cross_attn_hidden_text = nn.MultiheadAttention(embed_dim=self.hidden_state_dim , num_heads=16, batch_first=True, dropout=0.0)
            self.conv_img_embed = nn.Conv2d(in_channels=self.hidden_state_dim, out_channels=256, kernel_size=3, stride=2, padding=1)
            self.cross_attn_imgembed_hidden = nn.MultiheadAttention(embed_dim=256, num_heads=16, batch_first=True, dropout=0.0)
        
        if self.args.encoder_type == "vitembed":
            self.conv_img_embed = nn.Conv2d(in_channels=self.hidden_state_dim, out_channels=256, kernel_size=3, stride=2, padding=1)

        if self.args.encoder_type == "adapter_no_cross_attn":
            self.conv_img_embed = nn.Conv2d(in_channels=self.hidden_state_dim, out_channels=256, kernel_size=3, stride=2, padding=1)

        if self.args.encoder_type == "pixel_unshuffle_linear":
            self.shuffle_radio = 2
            self.linear_img_embed = nn.Linear(self.hidden_state_dim *(self.shuffle_radio **2), 256)
    ...
    def flatten_nlc(self, x):
        """[bs, c, h, w] -> [bs, hw, c]"""
        bs, c, h, w = x.shape
        return x.flatten(2).transpose(1, 2)

    def unflatten_nlc(self, x, h, w):
        """[bs, hw, c] -> [bs, c, h, w]"""
        bs, hw, c = x.shape
        return x.permute(0, 2, 1).view(bs, c, h, w)
    ...
    def forward(self, samples: NestedTensor, targets: List = None,
        ...
        bert_output = self.bert(**tokenized_for_encoder)  # bs, seq, 768
        encoded_text = self.feat_map(bert_output["last_hidden_state"])  # bs, seq, d_model=256
        ...
        # float cast
        if not self.args.deepspeed and images_hidden_states is not None:
        ...
        if self.args.llm_modelname == "InternVL2-2B" and self.args.encoder_type in ["upsample_cat", "hidden_states"]:
            ...
            for idx, layer in enumerate(self.fc_proj_shuffle):
                ...
                if position_embed and self.args.encoder_type == "hidden_states":
                    ...
                    ).bool().squeeze(1)  # [bs, h, w]

                    pe = self.backbone[1](
                        NestedTensor(hidden_states_temp, mask_temp)
                    ).to(hidden_states_temp.dtype)  # [bs, c_pos, h, w]
            ...
            if position_embed and self.args.encoder_type == "hidden_states":
                ...
                pe_final = self.backbone[1](
                    NestedTensor(src, mask_final)
                ).to(src.dtype)
        ...
        if self.args.encoder_type != "hidden_states":
            ...
            if self.num_feature_levels > len(srcs):
                ...
                for l in range(_len_srcs, self.num_feature_levels):
                    if l == _len_srcs:
                        src_l = self.input_proj[l](features[-1].tensors)
                    else:
                        src_l = self.input_proj[l](srcs[-1])
                    m = samples.mask
            ...
            if self.args.encoder_type == "upsample_cat":
                ...
                src_flat = self.flatten_nlc(srcs[0])  # [bs, HW, c]
                # cross_attn
                fused_src, _ = self.cross_attn_src_hidden(src_flat, hidden_states_0, hidden_states_0)
                # unflatten
        ...
        # ========== 2.5 encoder_type == "adapter" ==========
            ...
            img_embed_adapter = self.conv_img_embed(img_embed_adapter)
            img_embed_adapter = self.flatten_nlc(img_embed_adapter)
            adapter = img_embed_adapter

        # ========== 2.6 encoder_type == "img_embed_adapter" ==========
        elif self.args.encoder_type == "img_embed_adapter":
            img_embed_adapter, _= self.cross_attn_hidden_text(images_hidden_states[0], text_embeds[0], text_embeds[0])
            img_embed_adapter = self.unflatten_nlc(img_embed_adapter,hw[0][0],hw[0][1]) 
            img_embed_adapter = self.conv_img_embed(img_embed_adapter)
            img_embed_adapter = self.flatten_nlc(img_embed_adapter)
            src_0 = srcs[0]
            src_0 = self.flatten_nlc(src_0)  # [bs, hw, c]
            src_cross_attn, _ = self.cross_attn_imgembed_hidden(src_0, img_embed_adapter, img_embed_adapter)
            src_cross_attn = self.unflatten_nlc(src_cross_attn, srcs[0].shape[-2], srcs[0].shape[-1])
            srcs[0] = self.alpha * src_cross_attn + srcs[0]

        # ========== 2.7 encoder_type == "vitembed" ==========
        elif self.args.encoder_type == "vitembed":
            adapter = self.unflatten_nlc(images_hidden_states[0], hw[0][0], hw[0][1])
            adapter = self.conv_img_embed(adapter)
            adapter = self.flatten_nlc(adapter)


        # ========== 2.8 encoder_type == "adapter_no_cross_attn" ==========
        elif self.args.encoder_type == "adapter_no_cross_attn":
            img_embed_adapter = self.unflatten_nlc(images_hidden_states[0],hw[0][0],hw[0][1]) 
            img_embed_adapter = self.conv_img_embed(img_embed_adapter)
            img_embed_adapter = self.flatten_nlc(img_embed_adapter)
            adapter = img_embed_adapter


        elif self.args.encoder_type == "pixel_unshuffle_linear":
            img_embed_adapter = self.unflatten_nlc(images_hidden_states[0],hw[0][0],hw[0][1])
            hw[0][0], hw[0][1] = self.resize_hw(hw[0][0], hw[0][1], 2)
            img_embed_adapter = F.interpolate(img_embed_adapter, size=(hw[0][0], hw[0][1]), mode='bilinear').to(img_embed_adapter.device)
            img_embed_adapter = torch.nn.PixelUnshuffle(self.shuffle_radio)(img_embed_adapter)
            img_embed_adapter = self.flatten_nlc(img_embed_adapter)
            img_embed_adapter = self.linear_img_embed(img_embed_adapter)
            adapter = img_embed_adapter
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, cat_list, caption, return_indices=False):
        ...
        for j in range(len(cat_list)): # bs
            ...
            # if torch.isnan(for_match["pred_boxes"]).any():
            #     ipdb.set_trace()
...
class PostProcess(nn.Module):
    ...
    def __init__(self, num_select=100,text_encoder_type='text_encoder_type', nms_iou_threshold=-1,use_coco_eval=False,args=None) -> None:
        ...
        if args.eval_dataset == 'lvis':
            map_path = getattr(args, 'lvis_label_map', 'config/lvis_label_map.json')
            with open(map_path, 'r') as f:
                label_map_json = json.load(f)
            sorted_items = sorted(label_map_json.items(), key=lambda x: int(x[0]))
            cat_ids = [int(k) for k,_ in sorted_items]
            cat_list  = [v.replace('_',' ') for _,v in sorted_items]
            id_map = {idx: cid for idx,cid in enumerate(cat_ids)}
            print(id_map)
        elif args.use_coco_eval:
            from pycocotools.coco import COCO
            coco = COCO(args.coco_val_path)
            cat_ids = sorted(coco.getCatIds())
            cat_list = [c['name'] for c in coco.loadCats(cat_ids)]
            id_map = {idx: cid for idx,cid in enumerate(cat_ids)}
        else:
            cat_list = args.label_list
            id_map = {idx: idx for idx in range(len(cat_list))}

        caption = " . ".join(cat_list) + ' .'
        tokenized = self.tokenizer(caption, padding="longest", return_tensors="pt")
        label_indices = torch.arange(len(cat_list))
        pos_map_raw = create_positive_map(tokenized, label_indices, cat_list, caption)

        max_cid = max(id_map.values())
        pos_map = torch.zeros(max_cid+1, pos_map_raw.shape[-1])
        for local_idx,cid in id_map.items():
            pos_map[cid] = pos_map_raw[local_idx]
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name="groundingdino_eta")
def build_groundingdino_eta(args):
    device = torch.device(args.device)
    backbone = build_backbone(args)

    transformer = build_transformer_eta(args)

    dn_labelbook_size = args.dn_labelbook_size
    dec_pred_bbox_embed_share = args.dec_pred_bbox_embed_share
    sub_sentence_present = args.sub_sentence_present

    model = GroundingDINO_Eta(
        backbone,
        transformer,
