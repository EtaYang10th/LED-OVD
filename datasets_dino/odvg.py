    ...
    def __getitem__(self, index: int):
        ...
        while attempts < len(self.metas):
            ...
            if os.path.exists(abs_path):
                ...
                w, h = image.size
                if self.dataset_mode == "OD":
                    if len(meta["detection"]["instances"]) > 0:
                        anno = meta["detection"]
                        ...
                        for obj in instances:
                            if obj["category"] not in classes_text:
                                classes_text.append(obj["category"]) 
                        classes = [caption_dict[self.label_map[str(obj["label"])]] for obj in instances]
                        boxes = torch.as_tensor(boxes, dtype=torch.float32).reshape(-1, 4)
                        classes = torch.tensor(classes, dtype=torch.int64)
                    elif meta["detection"]["instances"] == []:
                        print(f"Image has no instances: {abs_path}")
                        index = (index + 1) % len(self.metas)
                        attempts += 1
                        continue
                    else:
                        ipdb.set_trace()
                    ...
                    if len(meta["grounding"]["regions"]) > 0:
                        anno = meta["grounding"]
                        ...
                        classes_text = []
                        for obj in instances:
                            if obj["phrase"] not in classes_text:
                                classes_text.append(obj["phrase"])
                        boxes = torch.as_tensor(boxes, dtype=torch.float32).reshape(-1, 4)
                        classes = torch.tensor(classes, dtype=torch.int64)
                        caption_list = uni_caption_list
                    elif meta["grounding"]["regions"] == []:
                        print(f"Image has no instances: {abs_path}")
                        index = (index + 1) % len(self.metas)
                        attempts += 1
                        continue
                ...
                target["sample_id"] = Path(meta["filename"]).stem
                #target["category"] = classes_text
...
def make_coco_transforms(image_set, fix_size=False, strong_aug=False, args=None):
    ...
    if image_set == 'train':
        ...
        if strong_aug:
            import datasets_dino.sltransform as SLT
    ...
    if image_set in ['val', 'eval_debug', 'train_reg', 'test']:
        ...
        if os.environ.get("GFLOPS_DEBUG_FLAG", False) == 'INFO':
